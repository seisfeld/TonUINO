...
const uint8_t msgSetupNewTagTrackAssigned = 107;  // 08
...
enum
{
  ...
  PUSH,
  RELEASE
}; // button states: UNCHANGED = 0, PUSH = 1, RELEASE = 2
...
enum
{
  ...
  B1P,
  B2P,
  B3P, // single button pushes, 1 -> buCount
  B1H,
  B2H,
  B3H,  // single button holds, buCount + 1 -> 2 * buCount
  ...
  IRC,
  IRM,
  IRP // ir remote events, 2 * buCount + 1 -> END
};
  ...
  if (nfcTag.playbackMode == 2)
  {
    ...
    if (directionForward)
    {
      ...
      else
      {
        ...
        if (isInteractive)
          Serial.println(F("mp3 | album mode -> end of folder -> ignore next track"));
        // stop playback
        else
        {
    ...
    else
    {
      ...
      // there are no more tracks before the current one, ignore the previous track command
        ...
        Serial.println(F("mp3 | album mode -> beginning of folder -> ignore previous track"));
    }
  }
    ...
    else
    {
      ...
      else
      {
        ...
        if (tempCookie != 0)
        {
          ...
          // For debug purposes output read information to serial port
          Serial.print(F("Track #"));
...
void loop()
{
  ...
  if (mfrc522.PICC_IsNewCardPresent() && mfrc522.PICC_ReadCardSerial() && !isLocked)
  {
    ...
    if (readNfcTagStatus == 1)
    {
      ...
      else if (nfcTag.cookie == 0)
      {
        ...
        if (nfcTag.playbackMode == 4)
        {
          ...
          mp3.playMp3FolderTrack(msgSetupNewTagTrackAssigned);
    ...
    else if (readNfcTagStatus == 253)
      Serial.println(F("nfc | read error"));
    else if (readNfcTagStatus == 254)
      Serial.println(F("nfc | authentication failed"));
    else if (readNfcTagStatus == 255)
      Serial.println(F("nfc | tag is not supported"));
    else
      Serial.println(F("nfc | unknown error"));
  }
